# -*- coding: utf-8 -*-
"""
字符串kmp算法, 详情见：https://time.geekbang.org/column/article/71845
"""
...
def get_next(pattern):
    ...
    *4. 递归的出口，就是在次长前缀子串的下一个字符和当前匹配 或 遇到-1，遇到-1则说明没找到任
    ...
    for i in range(1, m):
        ...
        # 根据上面跳出while的条件，当j=-1时，需要比较pattern[0](也即是pattern[j+1])和当前字符
        # 如果j!=-1，则pattern[j+1]和pattern[i]一定是相等的
        # 所以综合起来就是直接比较 pattern[j+1]和pattern[i] 就行
